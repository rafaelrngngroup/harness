...
- **Entity-consistent** — same clinic name, dentist name + CRO, and procedure definitions everywhere.
- **Corroborated** — consistent presence across ≥3 trusted sources (site + GBP + Doctoralia/health directory + YouTube).
- **Trust/E-E-A-T** — real dentist authorship, credentials, references; assistants are cautious with health topics and favor authoritative, non-sensational sources.

## Levers

1. **Structured data (Schema.org).** `Dentist`/`MedicalClinic` (name, CRO, address, specialty), `MedicalProcedure` per procedure page, `FAQPage` for patient FAQs, `LocalBusiness` NAP. High extraction value for AI.
...
3. **Definition-first, compliant claims.** ❌ "o melhor preenchimento, resultado garantido". ✅ "O preenchimento com ácido hialurônico repõe volume no sulco nasogeniano; resultados variam por paciente e exigem avaliação clínica individual."
4. **Entity coverage & consistency.** One-line compliant definitions for each procedure + a stable dentist/clinic entity (name + CRO) across site, GBP, Instagram, YouTube.
...
4. "Quem é um bom profissional para harmonização facial em [cidade]?"
...
## Compliance reminder
Everything surfaced to AI is public content — the CFO/ANVISA guardrails apply fully. Educational, factual, non-sensational content is simultaneously the most compliant **and** the most citable. See `compliance-guardrails.md`.
